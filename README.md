...
Построение графа выполняется однократно при инициализации, что обеспечивает высокую скорость ответа на запрос построения маршрута. Однако при очень большом количестве остановок инициализация может стать слишком долгой и стоит применить другой алгоритм.
...
Проект не требует внешних библиотек. Для сборки используйте любой С++20-совместимый компилятор.\
...
Разместите в корневой директории проекта файл input.json (название может отличаться), расширение может быть изменено на .txt, но содержимое должно быть валидным формату JSON, иначе будет выброшенно исклчение при чтении файла.
...
- **Route** - построение маршрута между двумя остановками (from и to). Если маршрут не найден вернёт пустой массив JSON.
...
```json
 ...
 Ответ на запрос **Route** может иногда отличаться от приведённого, если в графе существует несколько равнозначных маршрута с одинаковым временем. В этом случае программа может выбрать любой из них.
